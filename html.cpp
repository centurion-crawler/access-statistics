...
/* 首先在head内引入css和js头文件 初始化全局变量 
    chart 图表  
    datas 图表数据  
    colors 图表颜色  
    label 标签数组  
    Dname 图表展示的属性 
    title 图标中文名(Pie表的Dname有时候不会显示) 
    randomcolor 函数： 随机生成颜色RGB三通道
    paint 函数：js绘制图标 
        config 图表信息文件
        selectIndex 获取选择图表的类型
        ctx H5画布对象
        chart.destroy 删除chart内容
        当选取的图标类型变化时，先删除原来画布中chart的内容，再生成新的chart */
...
/* body */
   ...
   /*          maincontent   主页面              */
  /*         searchbox 实现必应搜索小功能        */
 /*   <li>标签 userinfo js接口显示当前用户信息  */
/*   putchart   js函数：多次显示chart         */
...
void showuser(int n, user A[])//显示之前访问的部分用户的信息
{
    for (int i=n-1;i>0;i-=n/(SHOWUSER-1))
    {
...
/* back部分 */
     ...
     /*     getBrowserinfo 通过接口获得用户IP       */
    /*       首先通过navigator方法获得UserAgent    */
   /*         对UserAgent进行文本解析             */
  /*     detectOS  获得用户的操作系统等信息       */
 /*            digits  获得用户系统位数         */
/*      通过AJAX传递信息到php,写入主机文件      */
//其他信息写入接口中

void htmlbackout(int n)
{
    printf("                           </table>\n\
                                    </div>\n\
                                </div>\n\
